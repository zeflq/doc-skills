...
<self-verification>
  <check>Domain index loaded before reading any code file.</check>
  <check>Context files matched semantically — not by keyword grep alone.</check>
  <check>No schema change implemented outside a migration.</check>
  <check>No new architectural pattern introduced without explicit user request.</check>
  <check>Post-task check completed — learning documented or justified as not needed.</check>
</self-verification>
```
